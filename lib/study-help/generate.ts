...
export async function generateIllustration(
  ...
  if (mode === "visualize") {
    ...
    const prompt = [
      ...
      "",
      "SPELLING & LABEL PLACEMENT (EXTREMELY IMPORTANT):",
      "- TRIPLE-CHECK every word in the image for correct spelling before finalizing. Misspelled labels destroy credibility.",
      "- Every arrow and leader line MUST point to the CORRECT element it labels. Verify each label points to the right part.",
      "- If a label says 'Head' the arrow must point to the head, not the arm. If it says 'Mitochondria' it must point to the mitochondria, not the nucleus.",
      "- Use minimal text — prefer short labels (1-3 words) to reduce spelling error risk.",
      "",
    ...
    const prompt = [
      ...
      "8. SPELLING: Triple-check every word for correct spelling. Copy text exactly as written in the original drawing. Every arrow must point to the correct element it labels.",
      "",
